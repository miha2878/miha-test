...
scrollTop:  $(anchor).offset().top-190
  ...
  $(window).scroll(function() {
    ...
    if ( top >= 96 ) {
    	$(".header-main").css({background: 'rgba(37, 43, 66, 1)', transition: "0.1s"});
    }else{
    	$(".header-main").css({background: 'inherit', transition: "0.1s"});
    }
  });

var owlone = $(".reviews__slider").owlCarousel({
        loop: true,
        autoplay: false,
        items: 1,
        autoWidth:false,
  ...
  $(document).ready(function() {
	var top = $(this).scrollTop();
    if ( top >= 96 ) {
    	$(".header-main").css({background: 'rgba(37, 43, 66, 1)', transition: "0s"});
    }else{
